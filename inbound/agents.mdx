---
title: "Agents"
description: "Deploy chatbots on your website, by phone, and in messaging apps to answer questions and capture leads."
icon: "robot"
---
...
> **Agents vs. AI Employees:** An [AI Employee](/overview/employees) is the assistant you configure and chat with inside the app. An **Agent** is a public-facing chatbot you embed on your website — each agent is *powered by* one of your AI employees but has its own system prompt, appearance, and lead-capture settings.

## What Are Agents?

Agents are customer-facing assistants powered by your AI employees. Unlike Chat — which is for you and your team — an agent talks to the outside world. They:
...
One agent can answer on several channels at once. Each channel is a tab in the agent editor, and each is enabled independently.

| Channel | What it does |
|---------|--------------|
| **Widget** | A chat bubble embedded on your website |
| **Webpage** | A standalone, shareable chat page — no embedding required |
| **SMS** | Replies to text messages sent to a connected number |
| **WhatsApp** | Replies to WhatsApp messages — see [WhatsApp](/inbound/agents/whatsapp) |
| **Voice** | Answers phone calls with a spoken voice, and can place outbound test calls |
| **Email** | Replies to messages sent to a connected mailbox |

The other tabs configure the agent rather than adding a channel:

- **General** — name, system prompt, knowledge base access, contact capture, and response delay
- **Integrations** *(Beta)* — let the agent use connected tools while it talks
- **Handoff** — hand a conversation to a human when the agent can't help
- **Notifications** — who gets told about new conversations and captured leads

### Voice agents

Voice agents answer a phone number on a connected telephony integration. You choose the voice and language, and can let the agent collect contact details during the call, send an SMS mid-call, or hand off to a person. Use **Test Call** on the agent row to have it call you so you can hear it before going live.

## Conversations

The **Conversations** item under Agents shows every conversation your agents have had, across all channels, in a two-panel reader: the list on the left, the full transcript on the right. Use it to check answer quality, find captured leads, and spot questions the agent handled badly.
...
Your agent can use your [Knowledge Base](/overview/knowledge-base) to answer questions accurately.
...
A: Configure your agent to offer human handoff. You can connect to your [Inbox](/engage/inbox) or other support systems.
...
A: Yes! Each agent can be powered by a different [AI Employee](/overview/employees).
